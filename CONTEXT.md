...
## SEO round (2026-09-19)
- Research and plan in docs/seo-2026-09-19/: R1 technical audit, R2 keywords and SERP, R3 local entity and Google Business Profile, R4 on what changed in search by 2026, and SEO-PLAN.md. The changes sit on branch `seo/metadata-2026-09-19`, to be rebased on Sidd's push and opened as a pull request on GitHub (maximizeGPT/spontaneous-cafe, connected to Vercel, so main deploys to production); nothing merges without Mohammed.
- Code: pure-Python image dimensions so the build no longer depends on Pillow, canonical host read from a Vercel environment variable at build time, hero posters moved into a real `<img>` so they get a srcset, one JSON-LD `@graph` per page in place of up to nine disconnected business objects, both forms of the `/privacy-policy` redirect fixed, robots meta and per-page og_alt added.
- Metadata only on the pages: titles, descriptions and og:image:alt rewritten on all seven indexable pages; body copy is unchanged because wording on the site is Sidd's (Mohammed, 2026-09-19). The wording suggestions from the research ("mushroom" in the foraging h1 and body, four pricing FAQs, a retreat catering section, the service-area lines) are in docs/seo-2026-09-19/COPY-PROPOSALS.md for Sidd to take or leave.
- Canonical host: `build.py` reads `VERCEL_PROJECT_PRODUCTION_URL` (fallback `SITE_URL`, then spontaneouscafe.com). Until cutover, `vercel.json` sends a host-matched `X-Robots-Tag: noindex` to the vercel.app alias. At cutover that header becomes a host-matched redirect to spontaneouscafe.com, and one Vercel redeploy flips every canonical, og:url, sitemap and robots line on its own.
- Deferred: a mushroom season calendar and a permits section (both need Matt's specifics), naming any farm stand (ask first), BreadcrumbList (needs a visible trail, a design change), opening hours and postal code in the schema (need Matt's real answers), sameAs for social accounts (need Matt's confirmation of ownership), and every marketplace listing (a business decision, not a code change).
- Off-site work: docs/seo-2026-09-19/OFFSITE-CHECKLIST.md, owned line by line between Mohammed and Matt.
- Standing rules: no Review or AggregateRating markup, a business may not mark up its own reviews; no doorway pages by town; no blog; no invented client fact goes on the site without a CONFIRM tag.
- 2026-09-21: merged with Sidd's SEO foundations and analytics commit b272f3e; FAQ markup removed by his commit, the FAQ text stays; his preview mode, tests and consent-gated analytics adopted.
