# Swap list: placeholders to replace before launch

Every image marked stock below is a free-licence stand-in (see `src/assets/MANIFEST.md` for source and licence). Replace with Matt's own photos, same filename, then delete the `ph` class from the `<img>`, `<figure>` or `.service__img` wrapper (`grep -rn 'class="[^"]*\bph\b' src/pages`).
...
|---|---|---|
| img/svc-foraging.jpg | Home service card | Matt with two boxes of chanterelles in the redwoods |
| img/svc-chef.jpg | Home service card | Bowl of salad and edible flowers on a deck rail |
| img/svc-catering.jpg | Home service card | Plated courses lined up on a kitchen counter |
| img/src-forest.jpg | Home sourcing grid, Foraging sub card | Black trumpets on a mossy trunk |
| img/mushrooms-basket.jpg | About sourcing grid, Classes sub card | Basket of chanterelles and hedgehogs |
...
Ask Matt before launch: the two cooks in `group-cooking.jpg` are not him, and their faces are visible. Confirm they are happy to appear on the site.

## Still on stock

| File | Role | What to ask Matt for |
|---|---|---|
| img/home-poster.jpg + video/home-pan.mp4 | Home hero | Hero-width photo or loop: hands at a pan over flame. A phone screenshot is 828 px wide, too small for a hero. |
| img/foraging-poster.jpg + video/foraging-moss.mp4 | Foraging hero | Hero-width moss and mushroom macro |
| img/chef-poster.jpg + video/chef-plating.mp4 | Private chef hero | Hero-width plating at a home table |
...
`grep -rn "CONFIRM" src/pages` lists every line that needs his yes. Prices with the "confirm with Matt" chip are private chef (from $500 a day, ingredients at cost) and catering (quoted per event).

## Other placeholders

- Formspree is live (form maeypdpl).
